...
\begin{frame}{Summary}
	\begin{itemize}
		...
		\vfill
		\item Reparametrization trick allows to get unbiased gradients w.r.t to variational posterior distribution.
		\vfill
		\item The VAE model is an LVM with an encoder network for $q(\bz | \bx, \bphi)$ and a decoder network for $p(\bx | \bz, \btheta)$.
		\vfill
		\item VAE is not a "true" bayesian model since parameters $\btheta$ do not have prior distribution.
		\vfill
