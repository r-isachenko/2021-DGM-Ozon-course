...
\begin{frame}{Bayesian framework}
	...
	\begin{block}{Meaning}
		...
		Posterior distribution $p(\bt | \bx)$ summarizes the knoweldge after the observations.
	\end{block}
\end{frame}
%=======
...
\begin{frame}{Flows intuition}
	\begin{itemize}
		...
		\item Then noise variable $\bz$ can be transformed to $\bx$ using inverse cdf~$F^{-1}$ ($\bx = F^{-1}(\bz)$).
	\end{itemize}
	How to transform random variable $\bz$  which has a distribution different from uniform to $\bx$?
		\begin{itemize}
			...
			\item Let $\bx \sim p(\bx)$ is a random variable with complex distribution $p(\bx)$ and cdf $F(\bx)$. 
			\item Then noise variable $\bz$ can be transformed to $\bx$ using cdf $G$ and inverse cdf~$F^{-1}$ ($\bx = F^{-1}(\bz_0) = F^{-1}(G(\bz))$).
		\end{itemize}
\end{frame}
%=======
