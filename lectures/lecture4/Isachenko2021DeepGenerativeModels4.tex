...
\begin{frame}{Forward KL vs Reverse KL}
	...
	\begin{block}{Monte-Carlo estimation}
		\vspace{-0.5cm}
		\[
			KL(\pi || p) = - \bbE_{\pi(\bx)} \log p(\bx | \btheta) + \text{const} \approx - \sum_{i=1}^n \log p(\bx_i | \btheta) \rightarrow \min_{\btheta}.
		\]
		\vspace{-0.5cm}
	\end{block}
	    \begin{block}{MLE problem}
...
\begin{frame}{Forward KL vs Reverse KL}
	\begin{block}{Forward KL}
	    \vspace{-0.3cm}
		\[
			\btheta^* = \argmax_{\btheta} \sum_{i=1}^n \log p(\bx_i | \btheta) \approx  \argmin_{\btheta} KL(\pi || p)
		\]
		Maximum likelihood estimation is equivalent of minimization of Monte-Carlo estimation of forward KL.
	\end{block}
	\begin{block}{Forward KL for flow model}
		...
		\item Until we do not want to sample from the flow, we could be not even able to compute $g(\bz, \btheta) = f^{-1}(\bz, \btheta)$.
		\end{itemize}
	\end{block}
\end{frame}
%=======
\begin{frame}{Forward KL vs Reverse KL}
	...
	\begin{block}{Reverse KL for flow model}
		...
		\item We could be not even able to compute $f(\bx, \btheta)$.
		\end{itemize}
	\end{block}
\end{frame}
%=======
...
\begin{frame}{Summary}
	...
	\vfill
	\item EM-algorithm and VAE model could be presented as a special case of mean field approximation.
	\vfill
	\item Forward KL minimization is equivalent to MLE. Reverse KL is used in variational inference.
	\vfill
	\item Flow models transform a simple base distribution to the complex one via a sequence of invertible transformations.
	\vfill
	\item Flow models has a tractable likelihood that is given by change of variable theorem.
	\vfill
	\item Planar flows is a simple form of invertible flow model (Sylvester flows are their extension). NICE model is a more powerful kind of flow.
	\end{itemize}
\end{frame}
\end{document} 